...
Read the shared [data-quality contract](data-quality.md) first. It defines the
portfolio gate, metric semantics and separate snapshot/last-refresh states.
...
Read refresh status without enqueueing work. A failed or unavailable refresh
must remain visible alongside the snapshot state; qualify its business date.
