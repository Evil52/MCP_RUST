...
Read [references/data-quality.md](references/data-quality.md) and
[references/report-contract.md](references/report-contract.md) before every
report. The shared contract governs identity, snapshot/refresh status and
cross-marketplace coverage. Also read:
...
2. For every included account, call both `ofk_collection_status` and
   `ofk_data_completeness` before requesting KPI details. Read
   `ofk_marketplace_sales_refresh_status` separately; preserve a failed update
   even when the last published snapshot is still usable. Name the refresh's
   business date rather than applying today's failure to an unrelated period.
