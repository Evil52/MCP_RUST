...
const CALL: &str = "SELECT wb_automation.recover_nexus_unconfirmed_bid(repeat('b',64),$1,repeat('c',64),repeat('d',64),$2::text::jsonb,'test/explicit-keep-current-bids')::text";
const BIDS: &str =
...
async fn audited_recovery_preserves_unknown_result_and_never_replays_write() {
    ...
    admin.batch_execute(r#"
      ...
      VALUES(repeat('a',64),'ofk_region_wb',40141836,repeat('c',64),clock_timestamp(),(clock_timestamp() AT TIME ZONE 'Europe/Moscow')::date,1,'{}','{}');
      ...
      VALUES(repeat('b',64),'ofk_region_wb',40141836,repeat('a',64),repeat('c',64),repeat('e',64),'change_bids','{"kind":"change_bids","changes":[{"nm_id":218972074,"from_bid_kopecks":602,"to_bid_kopecks":662}]}','reserved');
      UPDATE wb_automation.action_attempts SET status='write_started' WHERE idempotency_key=repeat('b',64);
      UPDATE wb_automation.action_attempts SET status='reconciliation_required',last_error_class='write_result_ambiguous' WHERE idempotency_key=repeat('b',64);
      ...
      FROM wb_automation.action_attempts WHERE idempotency_key=repeat('b',64);
    "#).await.unwrap();
    ...
    assert!(admin.batch_execute("UPDATE wb_automation.action_attempts SET status='cancelled',last_error_class='operator_closed_unconfirmed',readback_cycle_id=repeat('a',64) WHERE advert_id=40141836").await.is_err());
    ...
    admin.execute(r"
      INSERT INTO wb_automation.cycles(cycle_id,account_id,advert_id,policy_digest,observed_at,business_date,state_revision,snapshot_json,decision_json)
      VALUES(repeat('d',64),'ofk_region_wb',40141836,repeat('c',64),clock_timestamp(),(clock_timestamp() AT TIME ZONE 'Europe/Moscow')::date,1,
    ...
    let row=admin.query_one("SELECT status,write_started_at IS NOT NULL,readback_cycle_id,request_json FROM wb_automation.action_attempts WHERE idempotency_key=repeat('b',64)",&[]).await.unwrap();
    assert_eq!(row.get::<_, String>(0), "cancelled");
    assert!(row.get::<_, bool>(1));
    assert_eq!(row.get::<_, String>(2), "d".repeat(64));
    ...
    assert!(admin.batch_execute("UPDATE wb_automation.action_attempts SET status='write_started' WHERE idempotency_key=repeat('b',64)").await.is_err());
    assert!(
        writer
