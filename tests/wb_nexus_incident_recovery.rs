...
const CALL: &str = "SELECT wb_automation.recover_nexus_unconfirmed_bid(repeat('2',64),$1,repeat('3',64),repeat('4',64),$2::text::jsonb,'test/explicit-keep-current-bids')::text";
const BIDS: &str =
...
async fn audited_recovery_preserves_unknown_result_and_never_replays_write() {
    ...
    admin.batch_execute(r#"
      ...
      VALUES(repeat('1',64),'ofk_region_wb',40141836,repeat('3',64),clock_timestamp(),(clock_timestamp() AT TIME ZONE 'Europe/Moscow')::date,1,'{}','{}');
      ...
      VALUES(repeat('2',64),'ofk_region_wb',40141836,repeat('1',64),repeat('3',64),repeat('5',64),'change_bids','{"kind":"change_bids","changes":[{"nm_id":218972074,"from_bid_kopecks":602,"to_bid_kopecks":662}]}','reserved');
    ...
    assert!(admin.batch_execute("UPDATE wb_automation.action_attempts SET status='cancelled',last_error_class='operator_closed_unconfirmed',readback_cycle_id=repeat('1',64) WHERE advert_id=40141836").await.is_err());
    ...
    admin.execute(r"
      INSERT INTO wb_automation.cycles(cycle_id,account_id,advert_id,policy_digest,observed_at,business_date,state_revision,snapshot_json,decision_json)
      VALUES(repeat('4',64),'ofk_region_wb',40141836,repeat('3',64),clock_timestamp(),(clock_timestamp() AT TIME ZONE 'Europe/Moscow')::date,1,
