...
use super::{
    ControlAppConfig, ControlAuthConfig, OzonRuntimeIdentity,
    jwt::load_jwt_config,
    load_ozon_runtime,
...
impl ControlAppConfig {
    ...
    pub fn from_env() -> Result<Self> {
        Self::from_lookup_with_ozon_identity(
            |key| std::env::var(key).ok(),
            OzonRuntimeIdentity::Planner,
        )
    }

    /// Loads the same fail-closed Control configuration while binding the Ozon
    /// workflow worker to its distinct executor credential identity.
    pub(crate) fn from_ozon_executor_env() -> Result<Self> {
        Self::from_lookup_with_ozon_identity(
            |key| std::env::var(key).ok(),
    ...
    pub(super) fn from_lookup(mut lookup: impl FnMut(&str) -> Option<String>) -> Result<Self> {
        Self::from_lookup_inner(&mut lookup, OzonRuntimeIdentity::Planner)
    }

    #[cfg(test)]
    pub(super) fn from_lookup_for_ozon_executor(
        mut lookup: impl FnMut(&str) -> Option<String>,
    ) -> Result<Self> {
        Self::from_lookup_inner(&mut lookup, OzonRuntimeIdentity::Executor)
    }

    fn from_lookup_with_ozon_identity(
        mut lookup: impl FnMut(&str) -> Option<String>,
        identity: OzonRuntimeIdentity,
    ) -> Result<Self> {
        Self::from_lookup_inner(&mut lookup, identity)
    }

    fn from_lookup_inner(
        lookup: &mut dyn FnMut(&str) -> Option<String>,
        ozon_identity: OzonRuntimeIdentity,
    ) -> Result<Self> {
        let bind = value_or(lookup, "CONTROL_MCP_BIND", "127.0.0.1:8790")
            .parse::<SocketAddr>()
            .context("CONTROL_MCP_BIND должен иметь формат IP:PORT")?;
        let transport =
            value_or(lookup, "CONTROL_MCP_TRANSPORT", "http").parse::<TransportMode>()?;
        ...
        let auth_mode = value_or(lookup, "CONTROL_MCP_AUTH_MODE", "dev").parse::<AuthMode>()?;
        let auth = match auth_mode {
            AuthMode::Dev => {
                ...
                let allow_non_loopback = parse_strict_bool(
                    &value_or(lookup, "CONTROL_MCP_DEV_ALLOW_NON_LOOPBACK", "false"),
            ...
            AuthMode::Jwt => {
                ...
                ControlAuthConfig::Jwt(load_jwt_config(lookup)?)
            }
        };

        let policy_database = load_policy_database(lookup)?;
