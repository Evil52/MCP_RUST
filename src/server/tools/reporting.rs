...
impl OzonMcp {
    ...
    /// Читает снимок одного источника из PostgreSQL независимо от остальных. Для `seller_stocks` возвращает
    /// склады продавца всех типов: `delivery_type=1` — FBS. null — неизвестно; coverage — весь снимок.
