...
- Успешный ответ Ozon, Ozon Performance или WB ограничен 2 МиБ после распаковки; крупные периоды и
  ...
  ChatGPT передаёт модели оба поля, поэтому каждый результат занимает контекст дважды. При
  `MCP_TOOL_TEXT_CONTENT=summary` текстовый блок успешного результата заменяется короткой ссылкой на
  `structuredContent`; ошибки и результаты короче этой ссылки не меняются. Claude Desktop и Claude Code
  читают только текстовый блок, поэтому значение по умолчанию `json` сохраняет полную копию.
...
реестр — 105 инструментов. После выкладки этой версии обновите инструменты
...
ключи маркетплейсов и оставляет 17 инструментов PostgreSQL, реестра и локальной
очереди; прямые Ozon/WB-вызовы исключаются из диспетчера. Самостоятельный профиль
`compose.analytics-reader.yaml` требует JWT и ограниченные роли БД. Порядок
проверки изоляции и этапы финансовой истории описаны в
[плане безопасного расширения](docs/safe-analytics-expansion.md).

`ofk_wb_financial_ledger` читает только опубликованные финансовые пакеты WB
из PostgreSQL и доступен finance/admin в рамках разрешённого кабинета.
Доступ к новому методу проверен с ключами чтения; расписание и миграции
журнала активируются отдельно. `finance-collector` сохраняет страницы и
квоту между явными запусками. Для подготовленной себестоимости 1С добавлен
операторский `cost-import`: [формат и запуск](docs/1c-cost-import.md).

Для проверки рекламных решений на подготовленных данных доступен отдельный
`ads-optimizer recommend`: [контракт, пример и ограничения](docs/ads-optimizer.md).
Он рассчитывает ориентир среднего CPC и предложения по дневному бюджету
Ozon CPC: по целевой рекламной ДРР без 1С либо по ожидаемой экономике заказа,
сохраняя выбранную цель, причины и SHA-256 входных данных. Это offline
shadow-режим: он не подключается к кабинетам и не применяет рекомендации.
Команды `prepare`, `reconcile` и `--journal-dir` добавляют
[проверку экспорта MCP ОФК и журнал решений](docs/ads-optimizer-pipeline.md).


`ofk_wb_report_reconciliation` читает полные официальные отчёты и точную
сверку двух основных итогов из PostgreSQL. Поддержаны строки денежных
коэффициентов i128 и до 18 десятичных знаков; отсутствующий отчёт — N/D.
[Живой пилот ОФК](docs/finance-canary-20260913.md) прошёл на 4 642 строках.
