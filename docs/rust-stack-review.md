...
## Размеры после первого этапа (`af91371`)
...
## Второй локальный этап: общие типы и артефакт

Извлечён `mcp-marketplace-types`: идентификаторы кабинетов, marketplace enum
и контейнеры credentials. Клиенты Ozon больше не зависят от `config.rs`, а
конфигурация не импортирует credentials из WB-клиента. Старые публичные пути
сохранены через re-exports; загрузка секретов, валидация и авторизация не менялись.
После этого `config.rs` — 3,388 строк, `wb.rs` — 5,167. Их legacy budgets
уменьшены, сторонние версии зависимостей не обновлялись.

Повторная локальная проверка от 2026-09-09: полный workspace coverage-прогон
с временной PostgreSQL — 1,024 passed / 0 failed, 29 targets; покрытие
95.87% строк и 95.51% функций. Отдельно без `cfg(coverage)` прошли OAuth/MCP
и ambient-proxy wire tests. Новый crate отдельно проходит свои тесты; fmt,
строгий Clippy, rustdoc, file-growth gate, ShellCheck и dependency policy
также прошли. Пороги не ослаблены.

Добавлен [локальный сборщик артефакта](local-artifacts.md): только committed
source, нативный Linux Docker build, version probes, source SHA/tree и хеши
бинарников/архива. Четыре fixture-теста проверяют успешную упаковку с правильными
правами/хешами, исключение игнорируемых секретов, отказ при dirty tree, плохом
version probe и существующем архиве. Эти fixture-тесты используют подменный
Docker; фактическая release-сборка и проверка готового архива выполняются
отдельно после коммита. Push, CI-релиз и production rollout этим не подменяются.
