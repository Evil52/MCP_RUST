...
Это ещё не готовый к production сценарий первого запуска. Исходная пятёрка
содержит три предмета WB (4263, 6341, 7354); создание одной CPC-кампании для
неё недопустимо. Preflight теперь перечитывает карточки через существующий
...
(поддерживаются 7/9/11). Команда `start` явно останавливается до запроса
неподдерживаемой статистики и до записи. Нужен отдельно рассмотренный протокол
первого запуска с доказательством отсутствия расхода; отсутствие статистики
не превращается в нули и текущие защиты не отключаются. Для паузы 11 прежние
требования полной статистики и двух циклов сохраняются.
...
SKU; доступный `balance` читается только для `fund_and_start`.
