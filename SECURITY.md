...
- Verify the Analytics MCP tool list for this release contains exactly 114 stable tools, no preview tools,
...
- Verify `ofk_collection_status`, `ofk_data_completeness`, `ofk_metrics_history`,
  ...
  marketplace, generate a report, or send mail.
- Verify `ofk_request_ozon_sales_refresh` enforces actor/account RBAC before its separately
  restricted `report_refresh_requester` database boundary. Fourteen concurrent calls for one
  account must resolve to one active request ID with one creator. The role may execute only the
  request/status functions and must have no queue-table, snapshot-table, claim, completion, or
  marketplace credential access. Only the sequential `report-collector` may claim the job and
  atomically complete it with a full five-source Ozon snapshot. Database fencing must allow only
  one globally running refresh even with multiple collector replicas. The collector must reserve
  the complete 12-minute refresh deadline before scheduled report cutoffs and the 65-second Seller
  pacing tail after their completion windows.
